      ...
      {tab === "database" && (
        <>
          ...
          <div className="flex flex-col gap-3">
            ...
            {!cvLoading && cvDocs.length > 0 && (
              <div className="flex justify-center">
                <button
                  type="button"
                  disabled={selectedCvs.length === 0}
                  className="w-full rounded-full bg-primary px-6 py-2.5 text-sm font-medium text-white hover:opacity-90 transition-opacity cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed dark:text-black"
                >
                  {u.done ?? "Done"}
                </button>
              </div>
